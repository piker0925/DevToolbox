<template>
  <div class="mx-auto max-w-4xl px-6 py-8">
    ...
    <!-- 로그인 폼 -->
    <div v-if="!authed" class="rounded-xl border border-border bg-card p-6 shadow-sm">
    ...
    <div v-else class="flex flex-col gap-6">
      ...
      <section class="rounded-xl border border-border bg-card shadow-sm">
        <div class="flex items-center justify-between border-b border-border px-5 py-3">
          ...
          <button class="text-xs text-muted-foreground hover:text-foreground" @click="loadAll">새로고침</button>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tr v-if="stats.length === 0">
              <td class="px-5 py-4 text-muted-foreground" colspan="3">데이터 없음</td>
            </tr>
            <tr v-for="s in stats" :key="s.moduleId" class="border-b border-border last:border-0">
              <td class="px-5 py-2.5 font-mono text-xs text-foreground/80">{{ s.moduleId }}</td>
              <td class="px-5 py-2.5 text-foreground">{{ s.useCount }}</td>
              <td class="px-5 py-2.5 text-foreground">{{ s.likeCount }}</td>
            </tr>
            </tbody>
          </table>
        </div>
      ...
      <section class="rounded-xl border border-border bg-card shadow-sm">
        ...
        <ul class="divide-y divide-border">
          <li v-if="suggestions.length === 0" class="px-5 py-4 text-sm text-muted-foreground">건의사항 없음</li>
          <li v-for="s in suggestions" :key="s.id" class="px-5 py-3">
            <p class="text-sm text-foreground">{{ s.content }}</p>
            <p class="mt-0.5 text-xs text-muted-foreground">{{ formatDate(s.createdAt) }}</p>
          </li>
        </ul>
      </section>
      ...
      <section class="rounded-xl border border-border bg-card shadow-sm">
        ...
        <ul class="divide-y divide-border">
          <li v-if="comments.length === 0" class="px-5 py-4 text-sm text-muted-foreground">댓글 없음</li>
          <li v-for="c in comments" :key="c.id" class="flex items-start justify-between px-5 py-3">
            <div>
              <p class="text-xs text-muted-foreground">{{ c.moduleId }}</p>
              <p class="text-sm text-foreground">{{ c.content }}</p>
            </div>
            <button
                class="ml-4 shrink-0 rounded px-2 py-1 text-xs text-destructive hover:bg-destructive/10"
