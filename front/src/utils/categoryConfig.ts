...
import {AlignLeft, FileText, Globe, Image, Lock, Shuffle, Terminal, Type, Wrench, Zap,} from 'lucide-vue-next'
...
export const CATEGORY_CONFIG: Record<string, CategoryConfig> = {
    ...
    '보안': {icon: Lock as Component, bg: 'bg-red-50', color: 'text-red-600', thumbBg: 'bg-rose-600'},
    '포맷터': {icon: AlignLeft as Component, bg: 'bg-blue-50', color: 'text-blue-600', thumbBg: 'bg-blue-600'},
    '변환기': {icon: Shuffle as Component, bg: 'bg-cyan-50', color: 'text-cyan-600', thumbBg: 'bg-teal-600'},
    ...
    '유틸': {icon: Wrench as Component, bg: 'bg-slate-100', color: 'text-slate-500', thumbBg: 'bg-slate-500'},
}

export const CATEGORY_ORDER = [
    'PDF', '이미지', '생성기', '보안', '포맷터', '변환기', '텍스트', '네트워크', 'DevOps', '유틸',
]

export function getCategoryConfig(category: string): CategoryConfig {
    return CATEGORY_CONFIG[category] ?? CATEGORY_CONFIG['유틸']
}
