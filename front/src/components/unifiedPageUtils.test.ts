...
describe('detectFormat', () => {
    it('JSON 객체/배열을 감지한다', () => {
        expect(detectFormat('{"name": "DevToolbox", "active": true}')).toBe('json')
    ...
    it('XML을 감지한다', () => {
        expect(detectFormat('<root><name>DevToolbox</name></root>')).toBe('xml')
        expect(detectFormat('<?xml version="1.0"?>\n<a/>')).toBe('xml')
    })

    it('TOML을 감지한다 (key = value, [section])', () => {
        expect(detectFormat('name = "DevToolbox"\ntags = ["dev", "tools"]')).toBe('toml')
